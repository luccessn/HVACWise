...
export const CalctCards = ({ room, setEditingStructure, apt }) => {
  ...
  // თუ room.structures არ არსებობს, ან არ არის მასივი, მივანიჭოთ ცარიელი მასივი
  const structures = Array.isArray(room?.structures) ? room.structures : [];

  // ჯამური გამოთვლები
  const sumS27 = structures.reduce((sum, s) => sum + (s.s27 || 0), 0);
  const sumAg27 = structures.reduce((sum, s) => sum + (s.ag27 || 0), 0);
    ...
    <div className="p-4 border rounded-lg bg-gray-200">
      ...
      <div className="mb-4 p-3 rounded-lg bg-white shadow">
        ...
        <div className="grid grid-cols-2 gap-2 text-[15px] text-gray-700">
          <div>
            <span className="font-semibold">ფართობი (m²):</span> {room.m2 || 0}
          </div>
          <div>
          ...
          <div>
            <span className="font-semibold">ტემპერატურა:</span> {room.Temp || 0}
            °C
          </div>
        </div>
      </div>
      ...
      {structures.length === 0 ? (
        <div className="text-center  text-gray-600 text-lg italic py-3 bg-white rounded shadow-sm">
          ოთახი ცარიელია !
        </div>
      ) : (
              ...
              <div className="flex justify-between items-center">
                <div className="font-semibold text-[15px] p-2 text-gray-800">
                  ...
                  s.type === "მინის ჭერი"
                    ...
                    : `სიგრძე: ${s.width}მ x სიმაღლე: ${s.height}მ`}
                </div>

                {s.priort ? (
                  <div className="relative inline-block group">
                    <button className="relative hover:scale-110 px-10 top-2 text-sm font-semibold text-orange-600 rounded-xl transition-all duration-300 overflow-hidden">
                    ...
                    <div className="absolute invisible opacity-0 group-hover:visible group-hover:opacity-100 bottom-full left-1/2 -translate-x-1/2 mb-3 w-72 transition-all duration-300 ease-out transform group-hover:translate-y-0 translate-y-2 z-50">
                      <div className="relative p-4 bg-gradient-to-br from-orange-500/95 to-red-700/95 backdrop-blur-md rounded-2xl border border-white/10 shadow-[0_0_30px_rgba(79,70,229,0.15)]">
                        ...
                        <p className="text-sm text-gray-300">
                          ეს სტრუქტურა პრიორიტეტულია — ერთდროულად მხოლოდ ერთი
                          შეიძლება იყოს. თუ წაშლით, შემდეგში შეგიძლიათ ახალი
                          დაამატოთ.
                        </p>
                      </div>
                    </div>
                ...
                <button
                  onClick={() => {
                    if (confirm("ნამდვილად გსურს წაშლა?")) {
                      dispatch(
                        removeStructureFromRoomAction(apt.id, room.id, i)
                      );
                    }
                  }}
